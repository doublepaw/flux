...
| AWS v6 concurrent | EKS c6i.4xlarge, S3, all fixes (multipart + deadlock + key-collision) | **247** | 169 (tailing) | 1,136 / 2,259 | combined 339 MiB/s — matches GCP v3 concurrent (343): the architecture sets the number, not the cloud |
| AWS v6 4-broker | 5 nodes, 4 brokers, 4 bench shards co-scheduled on broker nodes | (unmeasurable) | — | 404-598 p50 | bench pods started up to 139s apart on the packed cluster → union-window aggregate meaningless; per-driver ~380 MiB/s, zero errors (fixes hold on S3). GCP v4 (drivers on dedicated nodes) is the clean scaling row |
